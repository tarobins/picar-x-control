...
check_robot_connection() {
    ...
    if ! ssh -o ConnectTimeout=3 -q "$ROBOT_HOST" exit; then
        log "Robot ($ROBOT_HOST) is OFFLINE. Starting Control Center in local SIMULATION MODE."
        ROBOT_ONLINE=false
    else
...
stop_services() {
    ...
    if [ "$ROBOT_ONLINE" = true ]; then
        ...
        log "Skipping remote server shutdown (robot offline)."
    fi
...
sync_code() {
    if [ "$ROBOT_ONLINE" = true ]; then
        ...
        log "Skipping code deploy (robot offline)."
    fi
}
...
start_services() {
    ...
    if [ "$ROBOT_ONLINE" = true ]; then
        ...
        log "Skipping SSH tunnels (robot is offline)."
    fi
    ...
    if [ "$ROBOT_ONLINE" = true ]; then
        ...
        log "Verification skipped (Simulation Mode)."
        success=true
    fi
